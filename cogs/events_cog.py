...
class EventsCog(commands.Cog):
    ...
    async def scrape_livescore_matches(self):
        """Récupère les matchs à venir via Browserless et parse le HTML avec la nouvelle structure."""
        if not BROWSERLESS_API_TOKEN:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(BROWSERLESS_CONTENT_API_URL, json=payload, timeout=45) as response:
                    if response.status != 200:
                        ...
                        print(f"⚠️ (BROWSERLESS) Erreur HTTP {response.status}: {error_text}")
                        return matches
            ...
            # On sélectionne directement tous les conteneurs de matchs programmés
            ...
            for container in match_containers:
                try:
                    ...
                    # Extraction directe de la date et de l'heure
                    ...
                    # Si la date est passée, on suppose que c'est l'année prochaine (pour les matchs de janvier en décembre)
                    ...
                    match_id = match_id_raw.replace('g_7_', '') # g_7_ pour handball
            ...
            print(f"❌ (BROWSERLESS) Erreur générale lors du scraping: {type(e).__name__}: {str(e)}")
        
        return []

    # Le reste du fichier (check_matches_loop, check_results_loop, etc.) reste identique
    # car il consomme le résultat de la fonction de scraping qui retourne le même format de données.
    ...
    async def check_rss_loop(self):
        ...
        try:
            ...
            if not feed.bozo:
                ...
                if self.first_check: self.first_check = False; print(f"ℹ️ (RSS) Premier check, {len(self.seen_articles)} articles ignorés")
                elif new_articles > 0: print(f"✅ (RSS) {new_articles} nouveaux articles envoyés")
                else: print("ℹ️ (RSS) Aucun nouvel article")
            else: print(f"⚠️ (RSS) Flux RSS invalide : {feed.bozo_exception}")
        except Exception as e:
    ...
    async def check_matches_loop(self):
        ...
        if not guild:
            print(f"❌ (MATCHES) Serveur {self.bot.guild_id} introuvable!"); return
        ...
        try:
            ...
            if not scraped_matches:
                print("ℹ️ (MATCHES) Aucun nouveau match trouvé via le scraping."); return
            ...
            if not journee_id:
                print("⚠️ (MATCHES) Impossible de déterminer la journée."); return
            existing_event_names = {event.name for event in guild.scheduled_events}
            pronos_cog = self.bot.get_cog('PronosticsCog'); new_matches_for_pronos = []
            for match in scraped_matches:
                ...
                if not database.get_match_by_event_id(match['event_id']) and event_name not in existing_event_names:
                    ...
                    try:
                        event = await guild.create_scheduled_event(name=event_name, description=f"Match de Starligue - Journée {journee_numero}\n\n📊 Faites vos pronostics !", start_time=match['start_time_utc'], end_time=match['start_time_utc'] + timedelta(hours=2), entity_type=discord.EntityType.external, location="Starligue Handball")
                        match_id = database.create_match(journee_id, match['event_id'], event.id, match['team1'], match['team2'], match['start_time_utc'])
                        if match_id: new_matches_for_pronos.append({'id': match_id, 'equipe1': match['team1'], 'equipe2': match['team2'], 'date_match': match['start_time_utc'], 'journee_numero': journee_numero})
                    except Exception as ex:
                        print(f"❌ (MATCHES) Erreur création événement : {ex}")
            if new_matches_for_pronos and pronos_cog:
                await pronos_cog.create_pronostic_messages_for_matches(new_matches_for_pronos)
        except Exception as e:
            print(f"❌ (MATCHES) Erreur critique boucle matchs: {e}")

    @tasks.loop(seconds=RESULTS_CHECK_INTERVAL)
    async def check_results_loop(self):
        print("🔍 (RESULTS) Vérification des résultats..."); now_utc = datetime.now(timezone.utc)
        matches_to_check = database.get_matches_to_check_results(now_utc - timedelta(hours=2))
        if not matches_to_check:
            print("ℹ️ (RESULTS) Aucun match à vérifier."); return
        ...
        for match_row in matches_to_check:
            ...
            try:
                ...
                if result and pronos_cog:
                    print(f"✅ (RESULTS) Résultat: {match['equipe1']} {result} {match['equipe2']}")
                    pronos_cog.process_match_result(match['id'], result)
                    await self.update_discord_event_with_result(match['discord_event_id'], match['equipe1'], match['equipe2'], result)
                    await asyncio.sleep(2)
            except Exception as e:
                print(f"❌ (RESULTS) Erreur vérification match {match['id']}: {e}")

    async def get_match_result(self, event_id):
        if not BROWSERLESS_API_TOKEN: return None
        match_url = f"https://www.livescore.in/fr/match/{event_id}/"; payload = {"url": match_url}
        try:
            async with aiohttp.ClientSession() as session:
            ...
            if status_elem and "Terminé" in status_elem.get_text():
                ...
                if home_score_elem and away_score_elem:
                    score1, score2 = home_score_elem.get_text(strip=True), away_score_elem.get_text(strip=True)
                    if score1.isdigit() and score2.isdigit(): return f"{score1}-{score2}"
        except Exception as e:
            print(f"❌ (RESULTS) Erreur scraping résultat {event_id}: {type(e).__name__}")
        return None
    ...
    async def update_discord_event_with_result(self, discord_event_id, team1, team2, score):
        ...
        except discord.NotFound: print(f"⚠️ (RESULTS) Événement Discord {discord_event_id} non trouvé.")
        except Exception as e: print(f"❌ (RESULTS) Erreur mise à jour événement {discord_event_id}: {e}")
    ...
    async def handball_command(self, ctx):
        guild = ctx.guild; upcoming_events = [e for e in guild.scheduled_events if e.creator == self.bot.user and e.status == discord.EventStatus.scheduled and "vs" in e.name]
        if not upcoming_events:
            await ctx.send("Aucun match programmé pour le moment.", ephemeral=True); return
    ...
    async def force_sync_command(self, ctx):
        await ctx.send("🔄 Synchronisation forcée des matchs...", ephemeral=True, delete_after=5)
        async with ctx.typing(): await self.check_matches_loop.coro(self)
        await ctx.send("✅ Synchronisation terminée!", ephemeral=True)

    @commands.command(name='checkresults', aliases=['cr'])
    @commands.has_permissions(administrator=True)
    async def check_results_command(self, ctx):
        await ctx.send("🔄 Vérification forcée des résultats...", ephemeral=True, delete_after=5)
        async with ctx.typing(): await self.check_results_loop.coro(self)
        await ctx.send("✅ Vérification terminée!", ephemeral=True)
